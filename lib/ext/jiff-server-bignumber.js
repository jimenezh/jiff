...
function secureRandom(max) {
  ...
  var multiple = magnitude.div(max).integerValue(BigNumber.ROUND_DOWN).times(max);

  var rand;
  do {
    rand = BigNumber.random(precision).times(magnitude).integerValue(BigNumber.ROUND_DOWN);
...
exports.make_jiff = function (base_instance, options) {
  ...
  jiff.compute = function (computation_id, options) {
    var computation_instance = base_compute.call(jiff, computation_id, options);
