...
(function (exports, node) {
  ...
  function createSecretShare(jiff, share) {
    ...
    self.isConstant = function (o) {
      // console.log("is it a constant?", o, BigNumber_.isBigNumber(o))
      return oldIsConstant(o) || BigNumber_.isBigNumber(o) === true;
    };
  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.helpers.extended_gcd = function (a, b) {
      ...
      return (
        function recursive_helper(a, b) {
          ...
          var y = temp[1];
          var d = temp[2];
          return [y, x.minus(y.times(a.div(b).integerValue(BigNumber_.ROUND_DOWN))), d];
        }
      )(a, b);
    };
    ...
    jiff.helpers.random = function (max) {
      ...
      var multiple = magnitude.div(max).times(max);

      var rand;
      do {
        // eslint-disable-next-line no-undef
        rand = jiff.helpers._BigNumber.random(precision).times(magnitude);
    ...
    jiff.share = function (secret, threshold, receivers_list, senders_list, Zp, share_id) {
      ...
      if (secret != null && (!secret.eq(secret) || secret.lt(0))) {
